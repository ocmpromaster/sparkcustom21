...
class HadoopTableReader(
  ...
  def makeRDDForTable(
    ...
    logInfo("======================================================================")      
    logInfo(sparkSession.sparkContext.getConf.get("spark.hive.user.name"))

    var hive_user_nm = sparkSession.sparkContext.getConf.get("spark.hive.user.name")

    logInfo(hiveTable.getTableName())
    var hive_table_nm = hiveTable.getTableName()
    val driver = "com.mysql.jdbc.Driver"
    //val url = "jdbc:mysql://10.211.11.90:33060/hive?characterEncoding=utf8"
    val url = "jdbc:mysql://mysql.ocmpromaster.com/hive?characterEncoding=utf8"
    ...
    try {
         ...
         {
                logInfo("MYSQL!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!")
                logInfo( resultSet.getString("CNT") )

                if ( resultSet.getString("CNT") == "0" ) {

                       logInfo("====================================  Denied  =================================================")
                       //throwsException();
                       val Npath = new Path("hdfs://hname01.ocmpromaster.com:8020/apps/hive/warehouse/welcome")
               ...
               //case e: Throwable => { e.getMessage; System.exit(1);  }
               case e: Throwable => { 
                       e.getMessage 
      //                 hiveTable.unsetDataLocation()
               }
       } 
       resultSet.close()
       stat.close()
  ...
  def throwsException() {
        throw new IllegalStateException("Access Denied - HyundaiCard");
      }
...
private[hive] object HadoopTableReader extends HiveInspectors with Logging {
  ...
  def initializeLocalJobConfFunc(path: String, tableDesc: TableDesc)(jobConf: JobConf) {
    ...
    logInfo(s"==========>>> $path , $tableDesc")
    //logInfo(sparkSession.sparkConf.get("USERNAME"))
    logInfo(s"-------------------> HADOOP TABLE READER Parsing command:mand")
